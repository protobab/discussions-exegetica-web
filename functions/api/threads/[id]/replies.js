...
export async function onRequestDelete({ env, request, params }) {
  ...
  if (!reply_id) return json({ error: 'reply_id required' }, 400)
  // Only author or admin can delete
  const ADMIN_USERS = ['eki']
  const reply = await env.DB.prepare('SELECT author_id FROM replies WHERE id = ?').bind(reply_id).first()
  if (!reply) return json({ error: 'Reply not found' }, 404)
  if (reply.author_id !== session.user_id && !ADMIN_USERS.includes(session.username)) {
    return json({ error: 'You can only delete your own replies' }, 403)
  }
  await env.DB.prepare('DELETE FROM replies WHERE id = ?').bind(reply_id).run()
  await env.DB.prepare('UPDATE threads SET reply_count = MAX(0, reply_count - 1) WHERE id = ?').bind(params.id).run()
  return json({ ok: true })
}
