...
export async function onRequestGet({ env, params }) {
  ...
  const session = await env.DB.prepare(`SELECT id, title, status, guest_name, description, cover_image, room_id, recording_url, recording_key, recording_key_mp4, zoom_link, listener_count FROM armchair_sessions WHERE id = ?`).bind(params.id).first()
