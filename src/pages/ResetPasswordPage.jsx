  ...
  return (
    <div style={{ minHeight:'80vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'40px 20px', background: 'var(--bg-page)' }}>
      <div style={{ background:'var(--surface-authcard)', backdropFilter:'blur(16px)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:16, padding:'38px 32px', width:'100%', maxWidth:420, boxShadow:'0 8px 40px rgba(0,0,0,0.5)' }}>
        ...
        {sub && <p style={{ fontFamily:F.body, fontSize:13.5, color:'var(--fg-5)', textAlign:'center', marginBottom:24 }}>{sub}</p>}
        {children}
      </div>
    </div>
  )
...
const inputStyle = { width:'100%', border:'1px solid rgba(201,168,76,0.25)', borderRadius:9, padding:'10px 13px', fontFamily:F.body, fontSize:14, outline:'none', boxSizing:'border-box', background:'var(--fg-06)', color:'var(--c-text)', marginBottom:14 }
    ...
    <AuthShell title="Reset Password" sub={done ? '' : 'Enter your new password below'}>
      {done ? (
        <div style={{ textAlign:'center' }}>
          ...
          <Link to="/login" style={{ display:'block', background:`linear-gradient(135deg,${C.gold},var(--c-gold-light))`, color:C.navy, borderRadius:10, padding:'12px', fontFamily:F.body, fontSize:14.5, fontWeight:700, textAlign:'center' }}>
        ...
        <>
          ...
          <button onClick={submit} disabled={loading||!token} style={{ width:'100%', background:`linear-gradient(135deg,${C.gold},var(--c-gold-light))`, color:C.navy, border:'none', borderRadius:10, padding:'12px', fontFamily:F.body, fontSize:14.5, fontWeight:700, cursor:'pointer', opacity:loading?0.7:1 }}>
            {loading ? 'Updating…' : 'Set New Password'}
          </button>
          <p style={{ fontFamily:F.body, fontSize:12.5, color:'var(--fg-35)', textAlign:'center', marginTop:14 }}>
